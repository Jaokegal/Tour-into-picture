function morphedImg = morph_to_rectangle(img, poly, outputSize)
    ...
    destX = [0, outputSize(2), outputSize(2), 0];
    destY = [0, 0, outputSize(1), outputSize(1)];

    % Ensure that the number of control points matches
    if numel(xCropped) ~= numel(destX) || numel(yCropped) ~= numel(destY)
        error('Number of control points must match.');
    end
    ...
    morphedImg = imwarp(croppedImg, tform, 'OutputView', imref2d(floor(outputSize)));
